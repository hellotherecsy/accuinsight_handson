...
(3) 작업
  - 작업은 모델 학습 작업을 구분하는 기준이 됩니다. 작업에는 내장 알고리즘을 사용하는 basic 모드와 직접 학습 코드를 작성하는 custom 모드가 있습니다. 작업 생성 후 작업 안에서 모델 학습을 진행합니다.
...
(1) 데이터 세트
  ...
  - 데이터 세트는 직접 업로드하는 raw데이터와 이 데이터로 전처리해 만드는 데이터가 함께 보관되는 단위입니다. 그러므로, 데이터를 업로드하면 데이터 세트를 함께 만들게 됩니다.
...
(2) 잘못 예측한 이미지
  ...
  오답 원인 추정: 원래는 호수 위 빙하 사진이지만, 흑백 처리하면서 바다 위에 떠 있는 빙하 사진처럼 보이게 됨
